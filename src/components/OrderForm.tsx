...
export default function OrderForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      // Save to localStorage for store owner demo retrieval
  ...
  return (
    <div className="bg-white rounded-3xl shadow-2xl border border-slate-250 overflow-hidden" id="order-form-container">
      {/* Header */}
            ...
            {(['XL', 'XXL', '3XL', '4XL'] as Size[]).map((sz) => {
              ...
              return (
                <button
                  ...
                  id={`size-pills-${sz}`}
                >
                  <span className="block text-xs sm:text-sm font-mono font-bold">{sz}</span>
